# Solution Design Document (Synthetic Example)

**Project:** Business Account Opening – KYC Document Verification (Automation)
**Document type:** SDD (Solution Design Document)
**Status:** SYNTHETIC — written for prototyping only, not a real client document.

## 1. Solution Summary
A UiPath automation ("KYC-Doc-Bot") will replace steps 2–7 of the manual
process described in the PDD. The bot polls the Business Onboarding Portal
...
via API/UI automation, and updates the case status.
...
2. **Document Extraction:** For each uploaded ID document, the bot uses the
   Document Understanding framework to classify the document type and
   extract: full name, date of birth, document expiry date, document number.
...
6. **Shareholding Comparison:** Bot compares declared shareholding
   percentages (from the application form) against PSC data; any individual
   >25% is tagged `EDD_REQUIRED = true`.
...
8. **Status Update:** Bot writes the outcome back to the Portal case:
   ...
   - Any other failed check → status "Manual Review Required," case routed
     to an Ops analyst queue with a note on which check failed.
...
- Structured log entry per case (fields: case ID, checks performed, results,
  timestamp) written to the automation's SQL log table.

## 5. Exception Handling (as designed so far)
- Document Understanding confidence below 80% on any extracted field →
  case routed to "Manual Review Required" queue with reason
  "Low OCR confidence."
...
---
*Note: intentionally silent on several points that a developer would need to
clarify — that gap is the point of this exercise, see the accompanying
"expected gaps" document.*
